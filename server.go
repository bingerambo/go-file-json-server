...
func (s Server) handleGet(w http.ResponseWriter, r *http.Request) {
	if !rePathFiles.MatchString(r.URL.Path) {
		...
		utils.WriteError(w, fmt.Errorf("\"%s\" is not found", r.URL.Path))
		return
	}
	if s.EnableCORS {
...
func (s Server) handlePost(w http.ResponseWriter, r *http.Request) {
	...
	defer srcFile.Close()
	logger.Debug(info)
	size, err := utils.GetSize(srcFile)
	if err != nil {
	...
	if size > s.MaxUploadSize {
		...
		utils.WriteError(w, errors.New("uploaded file size exceeds the limit"))
		return
	}
	...
	if written, err := dstFile.Write(body); err != nil {
		...
		w.WriteHeader(http.StatusInternalServerError)
		utils.WriteError(w, fmt.Errorf("the size of uploaded content is %d, but %d bytes written", size, written))
	}
	...
	utils.WriteSuccess(w, uploadedURL)
}
...
func (s Server) handlePut(w http.ResponseWriter, r *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	utils.WriteSuccess(w, r.URL.Path)
}
...
func (s Server) handleOptions(w http.ResponseWriter, r *http.Request) {
	...
	if rePathFiles.MatchString(r.URL.Path) {
		...
		w.WriteHeader(http.StatusNotFound)
		utils.WriteError(w, errors.New("not found"))
		return
	}
	w.Header().Set("Access-Control-Allow-Origin", "*")
